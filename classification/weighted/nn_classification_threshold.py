...
def optimize_thresholds(y_true, y_pred_prob, target_f1=0.3):
    """Find optimal thresholds for each class to balance precision and recall"""
    best_thresholds = [0.5, 0.5, 0.5]
    best_f1 = 0
    
    thresholds_to_test = [
        [0.5, 0.5, 0.1], [0.5, 0.5, 0.2], [0.5, 0.5, 0.3],
        [0.6, 0.6, 0.2], [0.6, 0.6, 0.3], [0.6, 0.6, 0.4],
        [0.4, 0.4, 0.2], [0.4, 0.4, 0.3], [0.4, 0.4, 0.4]
    ]
    
    for thresholds in thresholds_to_test:
        y_pred_prob_adjusted = y_pred_prob / thresholds
        y_pred_test = np.argmax(y_pred_prob_adjusted, axis=1)
...
class_thresholds, best_f1 = optimize_thresholds(y_test, y_pred_prob)

y_pred_prob_adjusted = y_pred_prob / class_thresholds
y_pred = np.argmax(y_pred_prob_adjusted, axis=1)

print(f"\nOptimized class thresholds: {class_thresholds}")
print(f"Expected macro F1 score: {best_f1:.3f}")
print(f"Class 0 threshold: {class_thresholds[0]}")
print(f"Class 1 threshold: {class_thresholds[1]}")
print(f"Class 2 threshold: {class_thresholds[2]} (optimized for rare class)")
