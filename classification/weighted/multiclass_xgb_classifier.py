...
weight_dict = {i: class_weights[i] for i in range(len(class_counts))}
...
print("\n=== Weighted Multiclass XGBoost Classification Report ===")
...
plt.ylabel('Actual')
plt.title('Confusion Matrix (Weighted Multiclass XGBoost)')
plt.tight_layout()
plt.show()
...
print(f"Subsample: {clf.subsample}")
print(f"Colsample by tree: {clf.colsample_bytree}")
...
plt.title('Multiclass ROC Curves (Weighted XGBoost)')
...
plt.title("Multiclass Precision-Recall Curves (Weighted XGBoost)")
plt.xlabel("Recall")
...
print(f"Balancing method: SMOTE + sample weights")
print(f"Model: XGBoost with 100 estimators")
